...
def getMutations(dataFile=None, genePos="mutPositions.csv"):
    ...
    # for geneName in ["GraR", "GraS", "MprF", "MsrR"]:
        mutsdict[geneName] = []
        ...
        for varPosition in posIDS:
            ...
            for index, row in subdfData.iterrows():
                ...
                # print(f"The genome input at {index} has {refBP} with {refBPCov} coverage for gene {geneName}")
            ...
            mutsdict[geneName].append({varPosition: str(altAA)})
    print(mutsdict)
    return mutsdict
    ...
    for gene in list(mutsdict.keys()):
        ...
        for mutposition in geneData:
            ...
            for posID in posids:
                ...
                altAA = mutPosdf[(mutPosdf["GENE"] == gene) & (mutPosdf["POS"] == posID)]["ALT"]
                print(altAA)
                altAA = "".join(altAA.values)
                
                colname = gene + "_" + str(int(posID)) + "_" + altAA
                print(f"the reference AA is {refAA} for gene {gene} at position {posID}")
                if AAdetected == refAA:
                    # print(f"no variant for gene {gene} at position {posID}")
                    dataval = 0
                elif AAdetected == "":
                    # print(f"No read detected for gene {gene} at position {posID}: {refAA} -> None")
                    dataval = None
                elif AAdetected == altAA:
                    # print(f"varaint detected for gene {gene} at position {posID}: {refAA} -> {AAdetected}")
                    dataval = 1
                else:
                    # print(f" Novel varaint detected for gene {gene} at position {posID}: {refAA} -> {AAdetected}")
                    dataval = AAdetected
                # print(dataval)
